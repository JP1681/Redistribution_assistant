    ...
    L.Control.BigImage = L.Control.extend({
        ...
        _createDownloadButton: function () {
            ...
            this._downloadBtn.addEventListener('click', () => {
                ...
                if ((!scale_value && scale_value != 0) || scale_value < this.options.minScale || scale_value > this.options.maxScale) {
        ...
        _print: async function () {
            let self = this;
            let s = parseInt(document.getElementById('scale').value);
            let scale = Math.pow(2,s);
            self.zoom = self._map.getZoom();
            var f = this.options.fileName;
